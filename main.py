...
for x in range(1, c.TURRET_LEVELS + 1):
    turret_sheet = pg.image.load(f"images/turrets/turret_{x}.png").convert_alpha()
    turret_spritesheets.append(turret_sheet)
# turrets
cursor_turret = pg.image.load("images/turrets/cursor_turret.png").convert_alpha()
# enemy
enemy_images = {
    "weak": pg.image.load('images/enemies/enemy_1.png').convert_alpha(),
    "medium": pg.image.load('images/enemies/enemy_2.png').convert_alpha(),
    "strong": pg.image.load('images/enemies/enemy_3.png').convert_alpha(),
    "elite": pg.image.load('images/enemies/enemy_4.png').convert_alpha()
}
# buttons
buy_turret_image = pg.image.load('images/buttons/buy_turret.png').convert_alpha()
cancel_image = pg.image.load('images/buttons/cancel.png').convert_alpha()
upgrade_turret_image = pg.image.load('images/buttons/upgrade_turret.png').convert_alpha()
begin_image = pg.image.load('images/buttons/begin.png').convert_alpha()
restart_image = pg.image.load('images/buttons/restart.png').convert_alpha()
fast_forward_image = pg.image.load('images/buttons/fast_forward.png').convert_alpha()
# gui
heart_image = pg.image.load('images/gui/heart.png').convert_alpha()
coin_image = pg.image.load('images/gui/coin.png').convert_alpha()
logo_image = pg.image.load('images/gui/logo.png').convert_alpha()
...
def create_turret(mouse_pos):
    ...
    if world.tile_map[mouse_tile_num] == 7:
        # check that there isn't already a turret present
        space_is_free = True
...
while run:
    ...
    if game_over == False:
        ...
        if placing_turrets == True:
            ...
            if cursor_rect[0] <= c.SCREEN_WIDTH:
                screen.blit(cursor_turret, cursor_rect)
            if cancel_button.draw(screen):
                placing_turrets = False
        # if turret is selected, show upgrade
        if selected_turret:
            # if turret can be upgrade show button
